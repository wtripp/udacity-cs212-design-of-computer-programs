...
def Pwin(me, you, pending):
    ...
    if me + pending >= goal:
        ...
        Proll = (1 - Pwin(you, me+1, 0) +
        sum(Pwin(me, you, pending+d)
            for d in (2,3,4,5,6))) / 6.
        
        return (Proll if not pending else
                max(Proll, 1 - Pwin(you, me+pending, 0)))
...
def strategy_d(state):
    ...
    if double == 1 and Pwin(me, you, pending) > 0.75:
        return 'double'
    
    # Accept a double only when opponent has a 75% chance of winning via optimal strategy.
    elif double == 'double':
        return 'accept' if Pwin(you, me, pending) > 0.75 else 'decline'
