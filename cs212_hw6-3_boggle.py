...
def boggle_words(board, minlength=3):
    ...
    for (i,sq) in enumerate(board):
        if sq != BORDER:
            find_boggle_words(board, rows, i, sq, minlength, results)
    return results

def find_boggle_words(board, rows, i, pre, minlength, results=None, visited=set()):
    ...
    if pre in PREFIXES:
        last = (i, board[i])
        visited.add(last)
        for nbr in neighbors(i, rows):
            if board[nbr] != BORDER and (nbr, board[nbr]) not in visited:
                find_boggle_words(board, rows, nbr, 
                                  pre+board[nbr], minlength, results, visited)
        visited.remove(last)
