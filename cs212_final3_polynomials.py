...
def poly(coefs):
    ...
    exps = range(len(coefs))
    x = 'x'
    
    terms = []
    for (c, e) in zip(coefs[::-1], exps[::-1]):
        if   c == 0: term = '0'
        elif e == 0: term = str(c)
...
def mul(p1, p2):
    ...
    start = 0
    for c1 in p1.coefs:
        for (i, c2) in enumerate(p2.coefs, start):
            coefs[i] += (c1 * c2)
        start += 1
...
def deriv(p):
    ...
    coefs = [i*c for (i,c) in enumerate(p.coefs[1:], 1)]
    return poly(tuple(coefs))

def integral(p, C=0):
    "Return the integral of a function p (with respect to its argument)."
    coefs = [C] + [(c+0.0)/i for (i,c) in enumerate(p.coefs, 1)]
    return poly(tuple(coefs))

def pad(c1, c2):
    "If number of coefs is unequal, zero-pad the shorter set to match the longer set."
...
def Poly(polystr):
    ...
    coefs = [coefmatch(term) for term in terms][::-1]
