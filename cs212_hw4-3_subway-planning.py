...
def subway(**lines):
    ...
    S = {}
    for line in lines:
        stops = lines[line].split()
        for i, stop in enumerate(stops):
            if stop not in S:
                S[stop] = {}
            if i > 0: # If first station, ignore stop to the left
                S[stop].update({stops[i-1]: line})
            if i < len(stops)-1: # If last station, ignore stop to the right
                S[stop].update({stops[i+1]: line})
    return S
...
def ride(here, there, system=boston):
    ...
    def ride_goal(state): return state == there
    def ride_successors(state): return system[state]
    return shortest_path_search(here, ride_successors, ride_goal)
...
def longest_ride(system):
    ...
    return max([ride(here, there, system)
               for here in system for there in system], key=len)
