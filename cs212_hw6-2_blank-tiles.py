...
for l in ascii_lowercase: points[l]
...
def row_plays(hand, row):
    ...
    hands = get_hands(hand) # <-- NEW: Get all hands, replacing blanks with each letter.
    for h in hands: # <-- NEW: Loop through each hand and accumulate results for each hand, h.
        for (i, sq) in enumerate(row[1:-1], 1):
            if isinstance(sq, set):
                ...
                if pre: ## Add to the letters already on the board
                    start = i - len(pre)
                    add_suffixes(h, pre, start, row, results, anchored=False)
                else: ## Empty to left: go through the set of all possible prefixes
                    for pre in find_prefixes(h):
                        if len(pre) <= maxsize:
                            start = i - len(pre)
                            add_suffixes(removed(h, pre), pre, start, row, results, anchored=False)
    return results

# NEW FUNCTION
def get_hands(hand):
    "Return a set of possible hands, accounting for blanks."
    ...
    for (i, sq) in enumerate(row[1:-1], 1):
        ...
        if sq == '*' or (is_empty(sq) and any(map(is_letter, neighborlist))):    
            if is_letter(N) or is_letter(S):   
                ...
                # UPDATED: convert letters in anchor to uppercase.
